...
说明:
  ...
  4. 交互式选择账号、输入目标 user id、可选输入附言，然后发送附件。
...
resolve_account_fields() {
  json_path=$1
  index=$2
  node --input-type=commonjs - "$json_path" "$index" <<'NODE'
const fs = require("fs");
const jsonPath = process.argv[2];
...
default_index=$(node --input-type=commonjs - "$CATALOG_JSON" <<'NODE'
const fs = require("fs");
...
account_line=$(resolve_account_fields "$CATALOG_JSON" "$account_index") || die "无效的账号序号: $account_index"

IFS='	' read -r ACCOUNT_ID ACCOUNT_NAME ACCOUNT_ENABLED BASE_URL API_KEY PRIVATE_PATH_TEMPLATE DEFAULT_TO <<EOF
$account_line
EOF
...
default_user_id=$(extract_user_default "$DEFAULT_TO")
TARGET_USER_ID=$(prompt "输入目标 VoceChat user id" "$default_user_id")
[ -n "$TARGET_USER_ID" ] || die "目标 user id 不能为空"

CAPTION=$(prompt "输入附言（可留空）")

if [ -f "$ATTACHMENT_INPUT" ]; then
  ATTACHMENT_FILE=$ATTACHMENT_INPUT
elif echo "$ATTACHMENT_INPUT" | grep -Eq '^https?://'; then
  ATTACHMENT_FILE=$(download_remote_attachment "$ATTACHMENT_INPUT")
else
  die "附件不存在，也不是有效的 http/https URL: $ATTACHMENT_INPUT"
fi
...
SEND_URL=$(build_send_url "$BASE_URL" "$PRIVATE_PATH_TEMPLATE" "$TARGET_USER_ID")
...
caption_response="$TMP_DIR/caption.json"
...
if [ -n "$CAPTION" ]; then
  send_text_message "$SEND_URL" "$API_KEY" "$CAPTION" "$caption_response"
fi

send_attachment_message "$SEND_URL" "$API_KEY" "$PAYLOAD_CONTENT_TYPE" "$UPLOAD_PATH" "$send_response"
MESSAGE_ID=$(parse_message_id "$send_response" || true)

log ""
log "发送成功"
log "  上传路径: $UPLOAD_PATH"
if [ -n "$MESSAGE_ID" ]; then
  log "  消息 ID: $MESSAGE_ID"
fi
