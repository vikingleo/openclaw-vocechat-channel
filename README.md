...
在 Telegram 中，面板命令第一次会新发一张管理卡片；之后的按钮操作会原地刷新。编辑类命令会直接写回宿主配置，并按通道配置自动热重载。路由、权限、账号详情卡片还提供主色复制命令按钮，可一键复制专用命令模板；权限卡片会直接展示当前管理员列表，并给每个管理员提供删除按钮；路由卡片和账号详情页提供 `user:` / `group:` 常用目标快捷按钮。
